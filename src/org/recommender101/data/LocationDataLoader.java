...
public class LocationDataLoader {
    ...
    public LocationDataLoader() {
        //aqui se puede agregar el cargado de los algoritmos para que creen el archivo con los ratings
    }
    ...
    public void applyConstraints(DataModel dm) throws Exception{
        ...
        if (this.sampleNUsers > -1) {
            ...
            Debug.log("DataLoader: Retaining " + dm.getUsers().size()
                    + " sampled usersIDs and " + dm.getRatings().size() + " ratings");
        ...
        do{
            ...
            if (minNumberOfRatingsPerUser > 0) {
                Debug.log("DataLoader: Applying minimum rating constraint for usersIDs ("+minNumberOfRatingsPerUser+")");
                int removed = Utilities101.applyMinRatingsPerUserConstraint(dm, minNumberOfRatingsPerUser);
                Debug.log("DataLoader: Removed "+  removed + " usersIDs. " + dm.getRatings().size()
                        + " ratings of " + dm.getUsers().size() + " usersIDs remain.");
                dm.removeDeadUsers();
                dm.recalculateUserAverages();
            }
...
//			// debugging here..
        ...
        Debug.log("LocationDataLoader:loadData: " + dm.getUsers().size() + " usersIDs and " + dm.getItems().size() + " items.");
        reader.close();
        applyConstraints(dm);
    }
    ...
    @R101Setting(displayName="Filename", description="Sets the filename of the dataset",
            type= R101Setting.SettingsType.FILE)
    ...
    @R101Setting(displayName="Minimum number of ratings per user",
            ...
            defaultValue="-1", type= R101Setting.SettingsType.INTEGER, minValue=-1, maxValue=Integer.MAX_VALUE)
    ...
    /**
     * Instructs the load to sample a given number of usersIDs
     * @param n how many usersIDs to keep
     */
    @R101Setting(defaultValue="-1", displayName="Sample N usersIDs",
            description="Instructs the loader to sample a given number of usersIDs", type= R101Setting.SettingsType.INTEGER,
     ...
     * @param d
     */
    @R101Setting(displayName="Density", description="Sets the density", type= R101Setting.SettingsType.DOUBLE,
     ...
     * @param b
     */
    @R101Setting(defaultValue="0", type= R101Setting.SettingsType.INTEGER, description="Sets the binarization method",
     ...
     * @param b
     */
    @R101Setting(description="All 0 ratings will be deleted", type= R101Setting.SettingsType.BOOLEAN,
     ...
     * @param value
     */
    @R101Setting(description="The string that separates columns in the input file", type= R101Setting.SettingsType.TEXT,
